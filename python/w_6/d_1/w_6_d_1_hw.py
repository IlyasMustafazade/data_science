...
def main():
    ...
    to_log_transform = np.array(["TotalIncome", "LoanAmount"])

    f_space = log_transform(f_space, to_log_transform)

    to_normalize = np.array(["TotalIncome", "LoanAmount", "Loan_Amount_Term"])
...
def train_eval_model(algo, data, predictor_arr, outcome_feature):
    ...
    predictor_data, outcome_data = \
        sampler.fit_resample(predictor_data, outcome_data)
    ...
    for train_index_arr, test_index_arr in stratified_k_fold.split(
        ...
        accuracy, f_1_score, conf_matrix = get_metrics(
            outcome_test_set, prediction)[:3]
...
def get_metrics(outcome_data, prediction, show_metrics=False):
    ...
    f_1_score = metrics.f1_score(outcome_data, prediction)

    conf_matrix = metrics.confusion_matrix(outcome_data, prediction)

    cl_report = metrics.classification_report(outcome_data, prediction)
    ...
    as_tuple = (accuracy, f_1_score, conf_matrix, cl_report)
...
def encode_col(f_space, encode_dict=None, col_arr=None, encoder_class=None):
    ...
    if (encode_dict is None) and (col_arr is None) and (encoder_class is None):
        ...
        encoder_arr = [encoder_class] * len_col_arr
    ...
    for i in range(len(col_arr_)):
        ...
        copy_f_space = col_to_df(copy_f_space, name, transformed)
...
def col_to_df(df, name, sub_df):
    ...
    if sub_df_n_col == 1:
        ...
        for i in sub_df_columns:
            ...
            new_col_name = name + "_" + str(clone_index)

            new_df.insert(insert_index, new_col_name, elem_array)
        ...
        raise Exception("Number of columns of sub_df must be more than 0")
    ...
    for i in f_space_columns:
        ...
        if elem.dtype == object:
            ...
            if len(elem.unique()) == count_lim:
                ...
                copy_f_space[i] = category_encoder.fit_transform(np.array(elem))
